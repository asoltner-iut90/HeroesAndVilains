...
export default {
  ...
  computed: {
    ...mapState(["CurrentTeam", "HeroPowerTypes"]),

  },

  methods: {

    ...mapActions(["getHero", "removeHeroesFromTeam", "updateHero"]),
    ...
    async removeHero(hero){
      ...
      this.$refs.ConfirmDialog.displayDialog("Suppression", "Voulez-vous vraiment enlever de l'équipe le héros "+ hero.publicName, () => {
...
<template>
  <div class="TeamView">
    ...
    <v-container v-else>
      <v-simple-table >
        <template v-slot:default>
          ...
          <tr v-for="(member,index) in CurrentTeam.members" :key="index">
            <td colspan="2" >
              <v-simple-table >
                <template v-slot:default>
                  ...
                  <tr>
                    <td colspan="3">
                        <v-list-item v-for="(power, index) in member.powers" :key="index">
                          <v-list-item-content>
                            <v-list-item-title>
                              {{power.name}} de type {{HeroPowerTypes[power.type-1]}} et niveau {{power.level}}
                            </v-list-item-title>
                          </v-list-item-content>
                        </v-list-item>
                    </td>
            ...
            <td>
              <v-btn color="warning" class="ma-3" @click="selectHero(member)" >Modifier</v-btn>
              <v-btn color="error" @click="removeHero(member)">Supprimer</v-btn>
            </td>
          </tr>
          </tbody>
      ...
      <EditHeroDialog :hero="selectedHero" ref="EditHeroDialog" @valid="update" />
